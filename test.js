...
function checkAnswer(userAnswer, correctAnswer) {
  ...
  let correctSymbol;
  const elementQuestion = questions.find(q => q.element === correctAnswer);
  if (elementQuestion) {
    correctSymbol = elementQuestion.symbol;
  } else {
    correctSymbol = correctAnswer; 
  }

  if (userAnswer === correctAnswer) {
    currentStreak++;
    score++;
    document.getElementById('feedback').innerText = `Correct! The symbol is ${correctSymbol}.`;
  } else {
    document.getElementById('feedback').innerText = `Incorrect! The correct symbol is ${correctSymbol}.`;
    currentStreak = 0;
  }
